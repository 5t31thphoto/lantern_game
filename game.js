...
function sky(m){m.innerHTML=shell('Your Sky','EVERY LIGHT YOU SENT IS STILL PART OF THE NIGHT',`<div class="bigSky"><div class="skyMoon"></div>${S.sky.map((x,i)=>`<button class="skyLantern ${x.color}" style="--x:${(8+(i*37)%84)}%;--y:${18+(i*29)%68}%" onclick="skyDetail(${i})">🏮</button>`).join('')}</div><div class="skyStats"><div><b>${S.sky.length}</b><small>lights released</small></div><div><b>${S.sky.filter(x=>x.message).length}</b><small>messages carried</small></div><div><b>${new Set(S.sky.map(x=>x.pattern)).size}</b><small>patterns made</small></div></div>${S.sky.length?'<p class="skyCaption">Some nights you can see them immediately. Some nights you have to look for the faintest ones.</p>':'<div class="empty"><div>☾</div><h2>The sky is waiting.</h2><p>Your first lantern will leave a mark here.</p><button class="primary" onclick="go('workshop')">Make one →</button></div>'}`)}
